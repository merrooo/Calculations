...
function drawArrow(cx, cy, angleDeg, r, color, label, isCurrent = false) {
  ...
  ctx.moveTo(x, y);
  ctx.lineTo(
    x - headlen * Math.cos(rad - Math.PI / 6),
    y - headlen * Math.sin(rad - Math.PI / 6),
  );
  ctx.lineTo(
    x - headlen * Math.cos(rad + Math.PI / 6),
    y - headlen * Math.sin(rad + Math.PI / 6),
...
function updateAngles() {
  ...
  const correctAngle =
    parseFloat(document.getElementById("correcttheta").value) || 0;
...
function calculateMT() {
  ...
  if (max > 0) {
    ...
    document.getElementById("loss_res").value = (
      0.25 * lf +
      0.75 * Math.pow(lf, 2)
    ).toFixed(4);
  }
...
function calculateCorrection() {
  ...
  const eff =
    (parseFloat(document.getElementById("eff_in").value) || 100) / 100;
  ...
  if (kwLoad > 0 && cos1 > 0 && cos2 > 0 && cos1 < 1) {
    ...
    let a1 =
      mode === "3"
        ? (P * 1000) / (Math.sqrt(3) * v * cos1)
        : (P * 1000) / (v * cos1);
    let a2 =
      mode === "3"
        ? (P * 1000) / (Math.sqrt(3) * v * cos2)
        : (P * 1000) / (v * cos2);
...
function updatePlot(P, Q1, Q2, s1, s2, Qc) {
  ...
  document
    .getElementById("polyOld")
    .setAttribute("points", `${x0},${y0} ${xP},${y0} ${xP},${y1}`);
  document
    .getElementById("polyNew")
    .setAttribute("points", `${x0},${y0} ${xP},${y0} ${xP},${y2}`);

  const qLine = document.getElementById("qCapLine");
  qLine.setAttribute("x1", xP);
  qLine.setAttribute("y1", y1);
  qLine.setAttribute("x2", xP);
  qLine.setAttribute("y2", y2);

  const tKW = document.getElementById("txtKW");
  tKW.setAttribute("x", (x0 + xP) / 2);
  tKW.setAttribute("y", y0 + 15);
  tKW.textContent = P > 0 ? P.toFixed(1) + " kW" : "";

  const tS1 = document.getElementById("txtKVAOld");
  tS1.setAttribute("x", (x0 + xP) / 2 - 20);
  tS1.setAttribute("y", (y0 + y1) / 2 - 10);
  tS1.textContent = s1 > 0 ? s1.toFixed(1) + " kVA" : "";

  const tS2 = document.getElementById("txtKVANew");
  tS2.setAttribute("x", (x0 + xP) / 2 - 10);
  tS2.setAttribute("y", (y0 + y2) / 2 + 15);
  tS2.textContent = s2 > 0 ? s2.toFixed(1) + " kVA" : "";

  const tQC = document.getElementById("txtQC");
  tQC.setAttribute("x", xP + 10);
  tQC.setAttribute("y", (y1 + y2) / 2);
  tQC.textContent = Qc > 0 ? Qc.toFixed(1) + " kVAR" : "";
}

function calculatePenalty() {
  let kwh =
    (parseFloat(document.getElementById("kwh_curr").value) || 0) -
    (parseFloat(document.getElementById("kwh_prev").value) || 0);
  let kvarh =
    (parseFloat(document.getElementById("kvarh_curr").value) || 0) -
    (parseFloat(document.getElementById("kvarh_prev").value) || 0);
...
const allConfigs = {
  ...
  "Jan2024-Jul2024": {
    residential: [
      ...
      { label: "ش5 (351-650)", p: 1.4, s: 15 },
      { label: "ش6 (651-1000)", p: 1.5, s: 25 },
      { label: "ش7 (1001+)", p: 1.65, s: 40 },
    ],
    commercial: [
      { label: "ش1 (0-100)", p: 0.65, s: 5 },
      { label: "ش2 (0-250)", p: 1.36, s: 15 },
      { label: "ش3 (0-600)", p: 1.5, s: 20 },
      { label: "ش4 (601-1000)", p: 1.65, s: 25 },
      { label: "ش5 (1001+)", p: 1.8, s: 40 },
    ],
    farmer: [{ label: "الري", p: 1.1, s: 4 }],
    factory: [{ label: "قوى محركة", p: 1.5, s: 15 }],
    lighting: [{ label: "انارة عامة", p: 1.5, s: 4 }],
    balance_cst: [{ label: "باقى المشتركين", p: 1.5, s: 15 }],
  },
  "2021-2023": {
    residential: [
    ...
    commercial: [
      { label: "ش1 (0-100)", p: 0.65, s: 5 },
      { label: "ش2 (0-250)", p: 1.2, s: 15 },
      { label: "ش3 (0-600)", p: 1.4, s: 20 },
      { label: "ش4 (601-1000)", p: 1.55, s: 25 },
      { label: "ش5 (1001+)", p: 1.6, s: 40 },
    ],
  ...
  "2019-2020": {
    residential: [
      { label: "ش1 (0-50)", p: 0.3, s: 1 },
      { label: "ش2 (51-100)", p: 0.4, s: 2 },
      { label: "ش3 (0-200)", p: 0.5, s: 6 },
      { label: "ش4 (201-350)", p: 0.82, s: 11 },
      { label: "ش5 (351-650)", p: 1.0, s: 15 },
      { label: "ش6 (651-1000)", p: 1.4, s: 25 },
      { label: "ش7 (1001+)", p: 1.45, s: 40 },
    ],
    commercial: [
      { label: "ش1 (0-100)", p: 0.65, s: 5 },
...
function onPeriodChange() {
  ...
  document.getElementById("currentPeriodHeader").innerText =
    "تشريح " + selectedPeriod;
  initTariffTable();
}
  ...
  if (type === "residential") {
    if (kwh >= 1001) tempTotal = kwh * list[6].p + list[6].s + 170;
      ...
      if (rem <= 150) tempTotal += rem * list[3].p + list[3].s;
        ...
        tempTotal +=
          150 * list[3].p + list[3].s + (rem - 150) * list[4].p + list[4].s;
    } else {
      if (kwh <= 50) tempTotal = kwh * list[0].p + list[0].s;
      else if (kwh <= 100)
        tempTotal =
          50 * list[0].p + list[0].s + (kwh - 50) * list[1].p + list[1].s;
      else
        tempTotal =
          100 * list[1].p + list[1].s + (kwh - 100) * list[2].p + list[2].s;
    }
  ...
  if (type === "residential") {
    if (totalKwh >= 1001) {
      ...
      if (rem <= 150) {
        ...
        document.getElementById("q5").value = (rem - 150).toFixed(2);
      ...
      if (totalKwh > 50) {
        ...
        if (totalKwh > 100) {
          document.getElementById("q3").value = (totalKwh - 100).toFixed(2);
    ...
    if (totalKwh >= 1001) {
      ...
      document.getElementById("r5").value = 140;
      document.getElementById("row5").classList.add("highlight");
      ...
      document.getElementById("r3").value = 50;
      document.getElementById("row3").classList.add("highlight");
    } else if (totalKwh >= 101) {
      document.getElementById("q2").value = totalKwh;
      document.getElementById("r2").value = 55;
      document.getElementById("row2").classList.add("highlight");
    } else {
...
function addToComparison() {
  ...
  const costCurrent = calculateCostForConfig(
    kwh,
    type,
    allConfigs[selectedPeriod],
  );
  ...
  row
    ...
    .addEventListener("input", updateComparisonTotals);
}
...
function updateLogTotals() {
  let tk = 0,
    ...
    tc = 0;
  document
    .querySelectorAll(".log-kwh")
    .forEach((el) => (tk += Number(el.innerText)));
  document
    .querySelectorAll(".log-cost")
    .forEach((el) => (tm += Number(el.innerText)));
  document
    .querySelectorAll(".log-actual")
    .forEach((el) => (ta += Number(el.innerText)));
  document
    .querySelectorAll(".log-diff")
    .forEach((el) => (td += Number(el.innerText)));
  document
    .querySelectorAll(".log-charge")
    .forEach((el) => (tc += Number(el.innerText)));
...
function updateComparisonTotals() {
  let totalKwh = 0,
    totalMoney = 0;
  document
    .querySelectorAll(".comp-kwh")
    .forEach((el) => (totalKwh += parseFloat(el.innerText) || 0));
  document
    .querySelectorAll(".comp-new")
    .forEach((el) => (totalMoney += parseFloat(el.innerText) || 0));
